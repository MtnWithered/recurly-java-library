...
/*    *//**
     ...
     *//*
    public GiftCard redeemGiftCard(final Long giftCardId, final String accountCode) {
        final GiftCard.Redemption redemptionData = GiftCard.createRedemption(accountCode);
        final String url = GiftCards.GIFT_CARDS_RESOURCE + "/" + Long.toString(giftCardId) + "/redeem";

        try {
            System.out.println(xmlMapper.writeValueAsString(redemptionData));
        } catch(Exception e){
        }
